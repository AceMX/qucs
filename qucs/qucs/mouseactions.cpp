...
void MouseActions::moveElements(Q3PtrList<Element> *movElements, int x, int y)
{
  ...
  for(pe = movElements->first(); pe != 0; pe = movElements->next()) {
    if(pe->Type == isWire) {
      ...
      if(((size_t)pw->Port1) > 3) {
	pw->x1 += x;  pw->y1 += y;
	if(pw->Label) { pw->Label->cx += x;  pw->Label->cy += y; }
      }
      else {  if((size_t)(pw->Port1) & 1) { pw->x1 += x; }
              if((size_t)(pw->Port1) & 2) { pw->y1 += y; } }

      if(((size_t)pw->Port2) > 3) { pw->x2 += x;  pw->y2 += y; }
      else {  if((size_t)(pw->Port2) & 1) pw->x2 += x;
              if((size_t)(pw->Port2) & 2) pw->y2 += y; }
...
void MouseActions::MMoveMoving(Schematic *Doc, QMouseEvent *Event)
{
  ...
  for(Element *pe=movingElements.first(); pe!=0; pe=movingElements.next()) {
    if(pe->Type == isWire) {
      ...
      if(((size_t)pw->Port1) > 3) { pw->x1 += MAx1;  pw->y1 += MAy1; }
      else {  if((size_t)(pw->Port1) & 1) { pw->x1 += MAx1; }
              if((size_t)(pw->Port1) & 2) { pw->y1 += MAy1; } }

      if(((size_t)pw->Port2) > 3) { pw->x2 += MAx1;  pw->y2 += MAy1; }
      else {  if((size_t)(pw->Port2) & 1) pw->x2 += MAx1;
              if((size_t)(pw->Port2) & 2) pw->y2 += MAy1; }
